...
if [ "$#" -ne 12 ]; then
    echo "Usage: $0 <version> <target-ref> <release-sha> <gateway-image> <worker-image> <ui-image> <gateway-digest> <worker-digest> <ui-digest> <gateway-attestation-url> <worker-attestation-url> <ui-attestation-url>" >&2
    exit 2
fi
...
worker_digest="$8"
ui_digest="$9"
gateway_attestation_url="${10}"
worker_attestation_url="${11}"
ui_attestation_url="${12}"
