...
// Problem: charmbracelet/x/cellbuf was pinned to a March 2025 pre-release pseudoversion that used the old charmbracelet/x/ansi v0.8 API (no-arg calls like
// b.Italic()). Go's MVS selected ansi@v0.11.7 (required by charm.land/lipgloss/v2 and golangci-lint/v2), which changed those method signatures to require a
// bool argument — causing the compile flood.

// Fix: Upgraded charmbracelet/x/cellbuf to v0.0.15 (released Feb 2026), which targets ansi@v0.11.5+ and uses the updated API. go build ./... and
// golangci-lint now run cleanly.
