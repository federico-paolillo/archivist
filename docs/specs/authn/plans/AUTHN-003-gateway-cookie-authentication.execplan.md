...
3. Define `AppCookieDefaults` with canonical `AuthenticationScheme`, `CookieName = "__Host-app-auth"`, and `SessionLifetime = TimeSpan.FromHours(24)` values.
4. Implement `AddAppCookie(this AuthenticationBuilder builder)` with default scheme `"app-cookie"` and the custom handler. Do not expose configurable cookie-name or session-lifetime options.
