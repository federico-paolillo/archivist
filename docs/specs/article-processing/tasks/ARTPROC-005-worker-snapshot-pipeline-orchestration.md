...
Implement the executable Worker processing pipeline that claims article-processing jobs, fetches HTML, stores `snapshot.html`, records terminal failure when snapshotting cannot continue, and reaches the downstream Markdown continuation boundary.

## Story / Context

As the deployed Worker process, I need `archivist-worker process` to invoke the article snapshot stage so a queued Telegram URL proceeds through claim, fetch, snapshot, and the downstream continuation boundary without committing premature success.
...
- Pipeline stages owned here: dequeue, resolve/fetch, snapshot write, snapshot failure terminal handling, and downstream continuation boundary.
- Snapshot-success behavior that updates `articles.canonical_url`, promotes `snapshot.html`, and reaches the Markdown continuation boundary without marking article/job success.
...
- Tests for executable processing, successful snapshot continuation, and ARC-coded snapshot-stage failures.
...
- Snapshot success atomically writes `snapshot.html`, updates `articles.canonical_url`, and reaches the Markdown continuation boundary.
...
- Snapshot success is only the continuation boundary into Markdown extraction; downstream features own Markdown extraction, summary generation, and terminal success.
